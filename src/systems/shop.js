...
export function buildTowerBuffItems(state) {
  const common = state.econ.tickets.common ?? 0;
  const rare   = state.econ.tickets.rare   ?? 0;
  const legend = state.econ.tickets.legend ?? 0;
  ...
  return [
    { id:"C_STREAK_PLUS",    icon:"🔥",  title:"리롤 스택 +1",
      desc:"연속 리롤 스택 +1.",
      cost:2, currency:"common",
      enabled:common>=2 && hasSel },

    { id:"C_ASPD_SHARD",     icon:"⚔️", title:`공속 파편 ${u?._aspdBuyCount??0}/3`,
      desc:"공격속도 +12% 영구.",
      cost:3, currency:"common",
      enabled:common>=3 && hasSel && (u?._aspdBuyCount??0)<3 },

    { id:"C_POWER_SHARD",    icon:"💥",  title:`공격 파편 ${u?._powerBuyCount??0}/3`,
      desc:"공격력 +10% 영구.",
      cost:3, currency:"common",
      enabled:common>=3 && hasSel && (u?._powerBuyCount??0)<3 },

    { id:"R_SPECIAL_CHARGE", icon:"✴️", title:"SPECIAL 충전",
      desc:"다음 리롤 SPECIAL PICK.",
      cost:2, currency:"rare",
      enabled:rare>=2 && hasSel },

    { id:"R_POWER_FORGE",    icon:"🔨",  title:`단련 ${u?._forgeBuyCount??0}/2`,
      desc:"공격력 +15% 영구.",
      cost:2, currency:"rare",
      enabled:rare>=2 && hasSel && (u?._forgeBuyCount??0)<2 },

    { id:"L_CRIT_AMP",       icon:"🗡️", title:`치명타 증폭 ${u?._critAmpCount??0}/2`,
      desc:"치명타 배율 +0.40 영구.",
      cost:1, currency:"legend",
      enabled:legend>=1 && hasSel && (u?._critAmpCount??0)<2 },

    { id:"L_POWER_AWAKEN",   icon:"🌠",  title:"전설 각성",
      desc:"공격력 +25% + 치명타율 +6%.",
      cost:2, currency:"legend",
      enabled:legend>=2 && hasSel && !u?._legendAwaken },
  ];
}
