 ...
 */
export function rollWaveType(stage, isBoss, ctx = {}) {
  ...
  const dangerSet = new Set([WAVE_TYPE.RUSH, WAVE_TYPE.PHANTOM, WAVE_TYPE.SIEGE]);
  const pool = allTypes.filter((t) => stage >= (minStage[t] || 1));
  if (pool.length <= 1) return pool[0] || WAVE_TYPE.NORMAL;

  // 특수 웨이브 비중은 유지하되 최대치를 낮춰 편차 완화
  const progress = Math.min(1, (stage - 5) / 30);
  const specialWeight = 0.20 + progress * 0.35; // 20%~55%

  const baseSpecial = specialWeight / Math.max(1, pool.length - 1);
  const weights = pool.map((t) => (t === WAVE_TYPE.NORMAL ? (1 - specialWeight) : baseSpecial));

  const lastWaveType = ctx.lastWaveType || null;
  const dangerStreak = Math.max(0, Number(ctx.dangerStreak || 0));

  for (let i = 0; i < pool.length; i++) {
    const t = pool[i];
    if (lastWaveType && t === lastWaveType) weights[i] *= 0.18;
    if (dangerStreak >= 1 && dangerSet.has(t)) weights[i] *= 0.45;
  }
