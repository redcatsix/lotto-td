...
import { createEconomyState, topUpCommonTickets, prepAidCommonForWave } from "../systems/economy.js";
import { ENEMY_TYPES, WAVE_TYPE, WAVE_TYPE_INFO, WAVE_STAT_MOD, rollWaveType, makeEnemy, pickEnemyTypeForWave } from "../entities/enemies.js";
import {
  UNIT_TYPES,
  UNIT_DEFS,
  ITEM_RARITY,
...
export async function initEngine() {
  ...
  function isDangerWaveType(wt) {
    return wt === WAVE_TYPE.RUSH || wt === WAVE_TYPE.PHANTOM || wt === WAVE_TYPE.SIEGE;
  }

  function waveTypeStatHint(wt) {
    const mod = WAVE_STAT_MOD[wt] || WAVE_STAT_MOD[WAVE_TYPE.NORMAL];
    if (!mod || wt === WAVE_TYPE.NORMAL) return "";
    const parts = [];
    if (mod.speedMul !== 1) parts.push(`속도 ${mod.speedMul > 1 ? "↑" : "↓"}`);
    if (mod.hpMul !== 1) parts.push(`체력 ${mod.hpMul > 1 ? "↑" : "↓"}`);
    if (mod.armorMul !== 1) parts.push(`장갑 ${mod.armorMul > 1 ? "↑" : "↓"}`);
    if (mod.lapReduce > 0) parts.push("랩 감소");
    return parts.join(" · ");
  }
  ...
  function updateWaveTypeUI() {
    ...
    if (isBoss) {
      ...
      const hint = waveTypeStatHint(wt);
      if (waveTypeDesc) waveTypeDesc.textContent = state.roundPhase === "PREP"
        ? `예고: ${info.desc}${hint ? ` · ${hint}` : ""}`
        : `${info.desc}${hint ? ` · ${hint}` : ""}`;
    }
  }
  ...
  function applyShiftForUpcomingStage() {
    ...
    state.nextWaveType = rollWaveType(state.stage, boss, {
      lastWaveType: state.lastWaveType,
  ...
  function grantStageRewards(stage, waveType = WAVE_TYPE.NORMAL) {
    ...
    // 어려운/특수 웨이브 보상 보정 (리스크 대비 리턴 강화)
    if (waveType === WAVE_TYPE.RUSH || waveType === WAVE_TYPE.PHANTOM) {
      state.econ.tickets.common += 1; gain.common += 1;
    }
    if ((waveType === WAVE_TYPE.ARMORED || waveType === WAVE_TYPE.SIEGE) && Math.random() < 0.35) {
      state.econ.tickets.rare += 1; gain.rare += 1;
    }
    if ((waveType === WAVE_TYPE.SPLIT || waveType === WAVE_TYPE.ELITE_VANGUARD) && Math.random() < 0.25) {
      state.econ.tickets.common += 1; gain.common += 1;
    }
    return gain;
  }

  function clearStage(stage, waveType = WAVE_TYPE.NORMAL) {
    const g = grantStageRewards(stage, waveType);
    const earlyAid = stage <= 5 ? topUpCommonTickets(state.econ, 2) : 0;
    const tag = `+C${g.common} +R${g.rare} +L${g.legend}`;
    const aidTag = earlyAid > 0 ? ` · 안전망 +C${earlyAid}` : "";
    state.msg = { text: `CLEAR!  ${tag}${aidTag}`, t: 1.2 };
    state.lastWaveType = waveType;
    if (isDangerWaveType(waveType)) state.dangerStreak = Math.min(3, (state.dangerStreak || 0) + 1);
    else state.dangerStreak = 0;
    state.stage += 1;
    ...
    const prepAid = prepAidCommonForWave(state.econ, state.nextWaveType, state.stage);
    if (prepAid > 0) {
      state.msg = { text: `${state.msg.text} · PREP 보급 +C${prepAid}`, t: 1.45 };
    }
  ...
  function updateWave(dt) {
    ...
    if (w.doneSpawning&&state.enemies.length===0) clearStage(w.stage, w.waveType || WAVE_TYPE.NORMAL);
  }
  ...
  function buildHelpHTML() {
    ...
    return `
      ...
      <ul>
        ...
        <li><b>안전망</b>: 초반/위험 PREP에서 일반 티켓 최소치 보정</li>
      </ul>
      <hr />
