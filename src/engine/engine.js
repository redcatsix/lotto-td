...
export async function initEngine() {
  ...
  // skill-detail removed (replaced by floating skill-node-panel)
  ...
  function syncTopUI() {
    ...
    // 섹션 내 서브 카운트 동기화
    const elSC=document.getElementById("shop-common-count"); if(elSC) elSC.textContent=String(state.econ.tickets.common);
    const elSR=document.getElementById("shop-rare-count");   if(elSR) elSR.textContent=String(state.econ.tickets.rare);
    const elSL=document.getElementById("shop-legend-count"); if(elSL) elSL.textContent=String(state.econ.tickets.legend);
    // ✅ 코어 HP 바 업데이트
    if (elCoreHpBar) {
  ...
  function renderShopPanel() {
    ...
    // 등급별 색상 정보
    const TIER_META = {
      common: { label:"일반", cls:"shop-item--common" },
      rare:   { label:"레어", cls:"shop-item--rare"   },
      legend: { label:"전설", cls:"shop-item--legend" },
    };
    ...
    function renderList(root, items) {
      ...
      root.innerHTML = items.map((it) => {
        const dis  = it.enabled ? "" : "disabled";
        const meta = TIER_META[it.currency] ?? TIER_META.common;
        const icon = it.icon ? `<span class="shop-icon">${it.icon}</span>` : "";
        const costBadge = `<span class="shop-cost-badge shop-cost-badge--${it.currency}">${meta.label} ${it.cost}</span>`;
        return `<button class="shop-item ${meta.cls}" data-shopid="${it.id}" data-cur="${it.currency}" ${dis}>`
          + `<div class="shop-item-top">${icon}<div class="shop-title">${safeText(it.title)}</div>${costBadge}</div>`
          + `<div class="shop-desc">${safeText(it.desc)}</div>`
          + `</button>`;
      }).join("");
    }
    renderList(shopCommonItems, commonItems);
    renderList(shopRareItems,   rareItems);
    renderList(shopLegendItems, legendItems);
  }
  ...
  function handleShopPurchase(id, currency) {
    ...
    if (currency === "common") {
      ...
      if (id === "C_TICKET_EXCHANGE") {
        ...
        state.msg = { text: "일반 5 → 레어 1 🔼", t: 0.8 }; syncTopUI(); return;
      }
      if (id === "C_ASPD_SHARD") {
        if (c < 3) return fail("일반 티켓 부족");
        const key = state.selectedKey;
    ...
    if (currency === "rare") {
      ...
      if (id === "R_CORE_SHIELD") {
        if (r < 2) return fail("레어 티켓 부족");
        if (state.coreShield) return fail("방어막이 이미 활성");
  ...
  function showNodePanel(nd) {
    ...
    panel.querySelector(".snp-close-btn")?.addEventListener("click", () => {
  ...
  function initPassiveTreeUI() {
    ...
    poeCanvas.addEventListener("wheel", (ev) => {
      ev.preventDefault();
      const rect  = poeCanvas.getBoundingClientRect();
      const ex    = ev.clientX - rect.left;
      const ey    = ev.clientY - rect.top;
      const delta = ev.deltaY > 0 ? 0.88 : 1.14;
      const newScale = Math.max(0.20, Math.min(2.5, poeScale * delta));
      // Keep the world point under cursor fixed
      poeOffX = ex - (ex - poeOffX) * (newScale / poeScale);
      poeOffY = ey - (ey - poeOffY) * (newScale / poeScale);
      poeScale = newScale;
      drawPoeTree();
    }, { passive: false });

    // ── Drag: pan ────────────────────────────────────────────────────────
    poeCanvas.addEventListener("pointerdown", (ev) => {
      poeDrag      = true;
      poeDragMoved = false;
      poeDragX     = ev.clientX;
      poeDragY     = ev.clientY;
      poeDragOX    = poeOffX;
      poeDragOY    = poeOffY;
      poeCanvas.setPointerCapture(ev.pointerId);
    });
    ...
    poeCanvas.addEventListener("pointermove", (ev) => {
      ...
      if (poeDrag) {
        const dx = ev.clientX - poeDragX;
        const dy = ev.clientY - poeDragY;
        if (Math.abs(dx) + Math.abs(dy) > 4) poeDragMoved = true;
        poeOffX = poeDragOX + dx;
        poeOffY = poeDragOY + dy;
        drawPoeTree();
        poeCanvas.style.cursor = "grabbing";
        return;
      }

      const nd = poeFindNode(ex, ey);
      if (nd?.id !== poeHoverNode?.id) {
        poeHoverNode = nd;
        drawPoeTree();
      }
      poeCanvas.style.cursor = nd ? "pointer" : "grab";
    });

    poeCanvas.addEventListener("pointerup", (ev) => {
      poeDrag = false;
      poeCanvas.style.cursor = "grab";
      if (poeDragMoved) return; // was a pan, not a click
